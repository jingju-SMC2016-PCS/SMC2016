...
class noteClass(object):
    ...
    def __init__(self):
        ...
        # this is we are using in classification
    ...
    def resetSegments(self):
        self.segments = []
        self.cumulDiffs = []    # store stdCadLe
        self.threshold = 0
    ...
    def segmentPointDetection1(self, threshold):
        ...
        #  we don't consider if there is only one or two extrema case
        if lenDiffAmp <= 3:
            return

        cumulDiff = [self.diffAmp[0]]
        self.cumulDiffs.append(np.std(cumulDiff))
    ...
    def segmentRefinement(self, notePt):
        ...
        if len(self.segments):                              #  note segmented by extrema
    ...
    def pltNotePtFc(self, x, y, p, rsquare, vibFreq, segCoef=0.2, saveFig=False, figFolder='./', figNumber=0):
        ...
        ######  pitchtrack figure #####
        # plot pitch contour along with stdCadLe
        f, ax = plt.subplots(2, sharex=True)

        ax[0].plot(x, y, 'b.', label='Note pitch contour')
        if len(self.ySmooth):
            ax[0].plot(x, self.ySmooth, 'b--', label='Smoothed pitch contour')

        if p is not None:
            pc = np.poly1d(p) # polynomial class
            ax[0].plot(x, pc(x), 'r-', label='fitting curve')
        ...
        if len(self.minimaInd) or len(self.maximaInd):
            # ax[0].plot(x[self.minimaInd], self.ySmooth[self.minimaInd], 'gv', markersize=10.0, label='Local minima')  #  markers of local minimas
            # ax[0].plot(x[self.maximaInd], self.ySmooth[self.maximaInd], 'g^', markersize=10.0, label='Local maxima')  #  markers of local maximas

            sortedIndices = sorted(self.minimaInd+self.maximaInd)
            ax[0].plot(x[sortedIndices], self.ySmooth[sortedIndices], 'gv', markersize=10.0, label='Local extrema')  #  markers of local minimas

            #  add text above the extreme
            # for ii in range(len(sortedIndices)):
            #     ax[0].annotate(str(ii), xy = (x[sortedIndices[ii]], self.ySmooth[sortedIndices[ii]]),
            #                 xytext=(x[sortedIndices[ii]], self.ySmooth[sortedIndices[ii]]+0.05))

            if len(self.segments):
                for seg in self.segments:
                    ax[0].axvline(x[self.extrema[seg]], color ='k', linestyle='-')

        # axarr[0].legend(loc='best')
        ax[0].set_ylabel('zero-mean\nmidi note number',fontsize = labelFontsize)
        ax[0].legend(loc='best', prop={'size':legendFontsize})
        # ax.set_title('rsquare: '+str(rsquare)+'vibratoFreq: '+str(vibFreq))

        if len(self.cumulDiffs):
            i_stdCadLe = np.hstack((x[sortedIndices], 1))
            ax[1].plot(i_stdCadLe, self.cumulDiffs, '--', label='StdCdLe')
            ax[1].plot(i_stdCadLe, self.cumulDiffs, 'gv', markersize=10.0, label='Local extrema positions')  #  markers of local minimas
            ax[1].axhline(segCoef, color = 'k', linestyle='-')
            ax[1].set_ylabel('midi note number', fontsize = labelFontsize)
            ax[1].set_xlabel('normalized time', fontsize = labelFontsize)
            ax[1].legend(loc='best', prop={'size':legendFontsize})
    ...
    def featureExtractionProcess(self,rpt,sbp,curvefittingDeg,pitchtrackNoteFolderPath,recordingName = 'unknown',figNumber = 0):
        ...
        # self.pltRefinedNotePtFc(xRpt, yRpt, p, rsquare, polyVar, vibFreq, saveFig=True,
        #                 figFolder=pitchtrackNoteFolderPath+ 'predict/refinedSegmentCurvefit/'+recordingName+'_curvefit_refined/',
        #                 figNumber = figNumber)
    ...
    def noteSegmentationFeatureExtraction(self,pitchtrackNoteFolderPath,featureVecFolderPath,
        ...
        ################### segmentation of pitch track by pYIN note transcription #####################################
        ...
        for rn in recordingNames:
            print 'processing: segCoef ',segCoef, ' recording name ', rn
            ...
            if evaluation:
                # coarse segmentation is the ground truth file
                coarseSegmentation_filename = os.path.join(pitchtrackNoteFolderPath,rn+'_coarseSeg.csv')
        ...
        #  use pitch track ptseg.pitchtrackByNotes from last step
            ...
            for ii in range(len(ptSeg.pitchtrackByNotes)):
                ...
                self.segmentPointDetection1(segCoef)             #  do the extrema segmentation here, method 1

                #  nc1.segmentPointDetection2()  # segmentation point
                # self.pltNotePtFc(x, y, None, None, None, segCoef, saveFig=True,
                #                  figFolder=pitchtrackNoteFolderPath+ 'predict/coarseSegmentCurvefit/'+rn+'_coarseCurvefit/',
                #                  figNumber=ii)

                self.segmentRefinement(pt)                       #  refined segmentation by extrama
                #print self.refinedNotePts

                for rpt in self.refinedNotePts:
                    # print jj

                    # extract features from the refined segments
                    featureVec,extrema,vibOut = self.featureExtractionProcess(rpt,sbp,curvefittingDeg,pitchtrackNoteFolderPath,rn,jj)
                    if featureVec[0]:
            ...
            if evaluation:
                # evaluate the refined segmentation
            ...
            featureFilename = os.path.join(featureVecFolderPath,rn+'.json')
            ...
            if predict:
                ...
                with open(os.path.join(pitchtrackNoteFolderPath,rn+'_refinedSegmentFeatures.json'), "w") as outfile:
        ...
        if evaluation:
            ...
            return COnOffF,COnF, COnOffP,COnP,COnOffR, COnR,OBOnRateGT,OBOffRateGT
