...
class pitchtrackSegByNotes(object):
    ...
    def readPyinPitchtrackTwoCols(self, pitchtrack_filename):
        ...
        pitchtrack = np.loadtxt(pitchtrack_filename,usecols=[0,1])
        frameStartingTime = pitchtrack[:,0]
        pitchtrack = pitchtrack[:,1]
    ...
    def readRefinedSegGroundTruth(self, refinedSeg_filename):

        '''
        :param refinedSeg_filename:
        :return: segStartingTime, segDurationTime
        '''

        seg = np.loadtxt(refinedSeg_filename,delimiter=',',usecols=[0,1,2])
        #monoNoteOut = np.loadtxt(monoNoteOut_filename, usecols=[0,1,2])

        segStartingTime = seg[:,0]
        segDurTime = seg[:,2]

        return segStartingTime, segDurTime
    ...
    def readCoarseSegmentation(self, coarseSegmentation_filename):
        ...
        coarseSeg = np.loadtxt(coarseSegmentation_filename,delimiter=',',usecols=[0])
    ...
    def refinedSegmentation(self,refinedSegmentation_filename):
        ...
        noteStartingTime, noteDurTime = self.readRefinedSegGroundTruth(refinedSegmentation_filename)
        noteEndingTime = noteStartingTime+noteDurTime
