...
class RefinedSegmentsManipulation(object):
    ...
    def eliminateOversegmentation(self,slopeTh = 10.0,pitchTh=2.0,boundaryTh=0.1,flatnoteSlope=20.0):
        '''
        eliminate over segmentation, concatenate
        ...
        while jj > 0:
            ...
            if self.representationTargetDict[secondKey]==0 and self.representationTargetDict[firstKey]==0:
                ...
                slope1 = np.arctan(slope1*slopeCoef)*180/np.pi
                slope2 = np.arctan(slope2*slopeCoef)*180/np.pi
                ...
                boundaryThSample = np.round(boundaryTh*self.nc1.samplerate/self.nc1.hopsize)

                # this coefs should be adjusted
                tooshortNote = 0.05
                ...
                if (abs(slope1)>flatnoteSlope and abs(slope2)>flatnoteSlope and slope1*slope2>0) or \
                    ...
                    if (slopeDiff<slopeTh or (abs(slope1)<flatnoteSlope and abs(slope2)<flatnoteSlope))\
                            and pitchDiff<pitchTh and boundaryDiff<boundaryThSample:
                        ...
                        #
                        # self.pltOverSegmentation(self.representationSegmentPts[firstKey],
                        #                          self.representationSegmentPts[secondKey],
                        #                          tempCombinedSegment,
                        #                          slope1, slope2,
                        #                          slopeDiff,
                        #                          len1,len2,
                        #                          saveFig=True,
                        #                          figNumber=str(jj-1)+'_'+str(jj))
    ...
    def pltOverSegmentation(self,firstSegment, secondSegment, combinedSegment,
                            slope1, slope2, slopeDiff, len1, len2, saveFig=False, figNumber='0'):
        '''
        plot the concatenation segments
        :param firstSegment:
        :param secondSegment:
        :param combinedSegment:
        :return:
        '''
        f, ax = plt.subplots(2, sharex=True)
        labelFontsize = 18

        len_fs = len(firstSegment)
        len_ss = len(secondSegment)
        len_cs = len(combinedSegment)

        hopsize_time = self.nc1.hopsize/self.nc1.samplerate

        x_fs = np.array(range(0,len_fs))*hopsize_time
        x_cs = np.array(range(0,len_cs))*hopsize_time
        x_ss = np.array(range(len_cs-len_ss,len_cs))*hopsize_time

        ax[0].plot(x_fs, firstSegment, label = 'Segment i')
        ax[0].plot(x_ss, secondSegment, 'r--', label = 'Segment i+1')
        ax[0].set_ylabel('midi note number', fontsize = labelFontsize)
        ax[1].plot(x_cs, combinedSegment, label = 'Concatenated segment')
        ax[1].set_ylabel('midi note number',fontsize = labelFontsize)
        ax[1].set_xlabel('time (s)',fontsize = labelFontsize)

        ax[0].legend(loc='best')
        ax[1].legend(loc='best')
        # ax[0].set_title('first len: '+"{0:.2f}".format(len1)+' '+
        #                 'second len: '+ "{0:.2f}".format(len2)+
        #                 'slope 1: '+"{0:.2f}".format(slope1)+
        #                 'slope 2: '+"{0:.2f}".format(slope2)+
        #                 'slopeDiff: '+"{0:.2f}".format(slopeDiff))

        if saveFig==True:
            # hardcode the path
            figFolder = '/Users/gong/Documents/MTG document/Conference/ISMIR2016/pitchContourSeg/01/predict/refinedSegmentCombined/'
            plt.savefig(figFolder+figNumber+'.png')
    ...
    def process(self, refinedSegmentFeaturesName,
        ...
        #  elimination
        self.eliminateOversegmentation(slopeTh = slopeTh,pitchTh=pitchTh,boundaryTh=boundaryTh,flatnoteSlope=flatnoteTh)
