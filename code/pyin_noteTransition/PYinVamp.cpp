...
PYinVamp::PYinVamp(float inputSampleRate) :
    ...
    m_pruneThresh(0.1f),
    m_weightPCD(0.5f),
//    m_fusionWeightStd(0.5f),
//    m_vibratoNoteThresh(0.5f),
//    m_vibratoNoteStdCoeff(1.0f),
    m_pitchProb(0),
    m_timestamp(0),
    m_level(0)
{
...
{
    return "Monophonic pitch and note tracking based on a probabilistic Yin extension. Incorporated bigram note transition probabilities of jingju singing.";
}
...
//    d.identifier = "weightPCD";
//    d.valueNames.clear();
//    d.name = "Pitch class distribution weight.";
//    d.description = "Final pitch of the note is weight*pitch_PCD + (1-weight)*pitch_notePitchContour.";
//    d.unit = "";
//    d.minValue = 0.0f;
//    d.maxValue = 1.0f;
//    d.defaultValue = 0.5f;
...
//    d.identifier = "fusionWeightStd";
//    d.valueNames.clear();
//    d.name = "Intonation histogram fusion standard deviation.";
//    d.description = "Large value increases the weight of far side overall pitch distribution peaks; less value introduces less fusion.";
//    d.unit = "";
//    d.minValue = 0.01f;
//    d.maxValue = 3.0f;
//    d.defaultValue = 0.5f;
...
//    list.push_back(d);
//    
//    d.identifier = "vibratoNoteThresh";
//    d.valueNames.clear();
//    d.name = "Vibrato note threshold.";
//    d.description = "If note length times this threshold < detected vibrato length, we correct its pitch.";
//    d.unit = "";
//    d.minValue = 0.2f;
//    d.maxValue = 1.0f;
//    d.defaultValue = 0.5f;
...
//    list.push_back(d);
//    
//    d.identifier = "vibratoNoteStdCoeff";
//    d.valueNames.clear();
//    d.name = "Vibrato note pitch correction coefficient.";
//    d.description = "Correct the vibrato note pitch to mean + vibratoNoteStdCoeff * std";
//    d.unit = "";
//    d.minValue = 0.0f;
//    d.maxValue = 2.0f;
//    d.defaultValue = 1.0f;
...
//    if (identifier == "weightPCD") {
//        return m_weightPCD;
//    }
//    if (identifier == "fusionWeightStd") {
//        return m_fusionWeightStd;
//    }
//    if (identifier == "vibratoNoteThresh") {
//        return m_vibratoNoteThresh;
//    }
//    if (identifier == "vibratoNoteStdCoeff") {
//        return m_vibratoNoteStdCoeff;
//    }
    return 0.f;
}
...
//    if (identifier == "fusionWeightStd")
//    {
//        m_fusionWeightStd = value;
//    }
//    if (identifier == "vibratoNoteThresh")
//    {
//        m_vibratoNoteThresh = value;
//    }
//    if (identifier == "vibratoNoteStdCoeff")
//    {
//        m_vibratoNoteStdCoeff = value;
//    }
...
//    std::vector<float> noteLevel;           // rms level of note
//    std::vector<double> noteVibSec;         // note vibrato section
//    std::vector<double> noteVibSecNonzero;  // note vibrato section without zero
    
    /* -------------------
     get the peaks of the PCD
     ---------------------*/
    
    /*
    // histogram of smoothedPitch
    vector<pair<double,double> > mpHistoOut = ih.histogram(smoothedPitchVector);
    vector<double> mpHistAmp;
    vector<double> mpHistBins;
    for (size_t iBin = 0; iBin < mpHistoOut.size(); ++iBin) {
        mpHistBins.push_back(mpHistoOut[iBin].first);
        mpHistAmp.push_back(mpHistoOut[iBin].second);
    }
    
    // peak detection parameters
    YinUtil::PeakDetectionParams params;
    params._minPos = 0.;
    params._maxPos = 1.;
    ...
    vector<pair<double, double> > peaks = YinUtil::peakDetection(mpHistAmp, params);
    
    size_t nWantedPeaks = size_t(std::fmin(peaks.size(), maxPeaks));
    vector<double> peakFreqs_mp(nWantedPeaks);
    
    for (size_t iPeak = 0; iPeak < nWantedPeaks; ++iPeak) {
        // convert freqs into bin frequency
        peakFreqs_mp[iPeak]    = peaks[iPeak].first*(mpHistBins.back() - mpHistBins[0])+mpHistBins[0];
    }
    */
...
//            noteLevel.push_back(1.0);
            ...
            if (oldIsVoiced == 1) // end of note
            {
                ...
                if (notePitchTrack.size() >= minNoteFrames)
                {
                    ...
                    /*-----------------
                     the commented code below is used for testing various techniques
                     such as, vibrato frequency correction, PCD weighting
                     These techniques are proved not effective.
                     ------------------*/
                    
                    // pitch distribution and vibrato correction
                    // convert type to double
//                    std::vector<double> notePitchTrackDouble(notePitchTrack.begin(), notePitchTrack.end());
                    /*
                    std::vector<double> noteLevelDouble(noteLevel.begin(), noteLevel.end());
                    
                    // detect vibrato section
                    noteVibSec = YinUtil::vibrato(notePitchTrackDouble, m_inputSampleRate, m_stepSize);
                    
                    // get non-zero vibrato section
                    int cumulateSum = 0;
//                    noteVibSecNonzero.resize(0);
                    for (size_t iiFrame = 0; iiFrame < noteVibSec.size(); ++iiFrame) {
//                        std::cerr << " noteVibSec " << noteVibSec[iiFrame] << std::endl;

                        if (noteVibSec[iiFrame]) {
                            cumulateSum += 1;
                            noteVibSecNonzero.push_back(noteVibSec[iiFrame]);
                        }
                    }
                    */
                    // pitch correction
//                    double  pitch_maxAmp_np = 0.0;
                    
                    /*
                    if (cumulateSum > m_vibratoNoteThresh * notePitchTrack.size()) {
                        
                        // if vibrato detected, we correct the note pitch
                        medianPitch = MathUtilities::mean(noteVibSecNonzero) +
                        m_vibratoNoteStdCoeff * MathUtilities::standardDeviation(noteVibSecNonzero);
                        
                    } else if (m_fusionWeightStd != 0){
                     
                    
                        
                        // or correct the note pitch by pitch distribution
                        // histogram of level weighted notePitchTrack
                        vector<pair<double,double> > npHistoOut = ih.histogram(notePitchTrackDouble);
                        vector<double> npHistoAmp;
                    
                        for (size_t iBin = 0; iBin < npHistoOut.size(); ++iBin) {
                            npHistoAmp.push_back(npHistoOut[iBin].second);
                        }
                    
                        // get max of the pitch distribution
                        double tempMax = 0.0;
                        for (size_t iBin = 0; iBin < npHistoOut.size(); ++iBin) {
                            if (npHistoOut[iBin].second > tempMax) {
                                tempMax = npHistoOut[iBin].second;
                                pitch_maxAmp_np = npHistoOut[iBin].first;
                            }
                        }
                    
                        // mp nearest peak
                        double pitch_nearestPeak_mp = mpHistBins.back();
                        double pitchDistance_nearest = mpHistBins.back();
                        for (size_t iPeak = 0; iPeak < peakFreqs_mp.size(); ++iPeak) {
//                            std::cerr << std::fabs(peakFreqs_mp[iPeak]-pitch_maxAmp_np) << std::endl;
                            if (std::fabs(peakFreqs_mp[iPeak]-pitch_maxAmp_np) <= pitchDistance_nearest) {
                                pitch_nearestPeak_mp = peakFreqs_mp[iPeak];
                                pitchDistance_nearest = std::fabs(peakFreqs_mp[iPeak]-pitch_maxAmp_np);
                            }
                        }

                    
//                    for (size_t iPeak = 0; iPeak < freqs.size(); ++iPeak) {
//                    }
                    
                    
                        // fusion of smoothed pitch track and note pitch track with a weighting distribution
                        vector<pair<double,double> > mpnpHistoFusion =
                        ih.histogramFusion(mpHistoOut, npHistoOut, m_fusionWeightStd);
                        
                        // get the median pitch as the max of the pitch distribution
                        double tempMax = 0.0;
                        for (size_t iBin = 0; iBin < mpnpHistoFusion.size(); ++iBin) {
                            if (mpnpHistoFusion[iBin].second > tempMax) {
                                tempMax = mpnpHistoFusion[iBin].second;
                                medianPitch = (float)mpnpHistoFusion[iBin].first;
                            }
                        }
                        ...
                        for (size_t iBin = 0; iBin < npHistoOut.size(); ++iBin) {
                            if (npHistoOut[iBin].second > tempMax) {
                                tempMax = npHistoOut[iBin].second;
                                medianPitch = (float)npHistoOut[iBin].first;
                            }
                        }
                ...
                */
                        // -- original pYIN code of searching median pitch
                        std::sort(notePitchTrack.begin(), notePitchTrack.end());
                        float medianPitch = notePitchTrack[notePitchTrack.size()/2];
//                    }
                    
//                    float medianPitch = float(pitch_nearestPeak_mp)*m_weightPCD + float(pitch_maxAmp_np)*(1-m_weightPCD);
